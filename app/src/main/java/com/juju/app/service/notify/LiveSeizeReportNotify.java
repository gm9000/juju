...
public class LiveSeizeReportNotify extends BaseNotify<SeizeNotifyEvent.SeizeNotifyBean> {

    private Logger logger = Logger.getLogger(LiveSeizeReportNotify.class);

    private volatile static LiveSeizeReportNotify inst;

    public static LiveSeizeReportNotify instance() {
        if(inst == null) {
            synchronized (LiveSeizeReportNotify.class) {
                if (inst == null) {
                    inst = new LiveSeizeReportNotify();
                }
            }
        }
        return inst;
    ...
    public void executeCommand4Send(SeizeNotifyEvent.SeizeNotifyBean seizeNotifyBean) {
        sendLiveSeizeReportToMServer(seizeNotifyBean);
    }
    ...
    public void onEvent4BusinessFlowSendEvent(SeizeNotifyEvent.BusinessFlow.SendParam sendParam) {
        switch (sendParam.send) {
            case SEND_LIVE_SEIZE_REPORT_MSERVER_OK:
...
//                triggerEvent(externalEvent);
                break;
            case SEND_LIVE_SEIZE_REPORT_MSERVER_FAILED:
                break;
        }
    }

    public void sendLiveSeizeReportToMServer(final SeizeNotifyEvent.SeizeNotifyBean seizeNotifyBean) {
        ...
        notifyMessage4Group(peerId, message,
                IMBaseDefine.NotifyType.RELAY_COUNT, uuid, true,
                    ...
                    public void onSuccess(Object t) {
                        ...
                        if(t instanceof XMPPServiceImpl.ReplayMessageTime) {
                            ...
                            buildAndTriggerBusinessFlow4Send(SeizeNotifyEvent.BusinessFlow.SendParam
                                    .Send.SEND_LIVE_SEIZE_REPORT_MSERVER_OK, seizeNotifyBean);
                        } else {
                            buildAndTriggerBusinessFlow4Send(SeizeNotifyEvent.BusinessFlow.SendParam
                                    .Send.SEND_LIVE_SEIZE_REPORT_MSERVER_FAILED, seizeNotifyBean);
                        }
                    }
    ...
    public void onEvent4BusinessFlowRecvEvent(SeizeNotifyEvent.BusinessFlow.RecvParam recvParam) {
        switch (recvParam.recv) {
            case PROCESS_LIVE_SEIZE_REPORT_OK:
                SeizeNotifyEvent externalEvent = new SeizeNotifyEvent(SeizeNotifyEvent.Event.LIVE_SEIZE_REPORT_OK, recvParam.bean);
                triggerEvent(externalEvent);
                break;
            case PROCESS_LIVE_SEIZE_REPORT_FAIL:
...
//                });
    ...
    public void synLocalLiveData(final SeizeNotifyEvent.SeizeNotifyBean seizeNotifyBean) {
        ...
        buildAndTriggerBusinessFlow4Recv(SeizeNotifyEvent.BusinessFlow.RecvParam
                .Recv.PROCESS_LIVE_SEIZE_REPORT_OK, seizeNotifyBean);
    }
