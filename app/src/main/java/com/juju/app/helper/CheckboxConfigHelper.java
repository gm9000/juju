...
public class CheckboxConfigHelper {
	...
	public void initForbiddenCheckBox(final CheckBox checkBox,
									  final String sessionKey, final String groupId) {
		...
		checkBox.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				configMgr.setSession4Forbidden(sessionKey, groupId, checkBox.isChecked());
			}
		});
		configCheckBox(checkBox, sessionKey, ConfigurationSp.CfgDimension.NOTIFICATION);
	...
	private void handleCheckBoxChanged(final CheckBox checkBox,final String key,
									   final ConfigurationSp.CfgDimension dimension) {
		if (checkBox == null || configMgr == null) {
			return;
		}
