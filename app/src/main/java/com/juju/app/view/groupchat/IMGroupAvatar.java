...
public class IMGroupAvatar extends LinearLayout {
    ...
    public IMGroupAvatar(Context context, AttributeSet attrs) {
        ...
        parentPadding = ScreenUtil.dip2px(getContext(), DEFAULT_PADDING_DIP);
        childMargin = ScreenUtil.dip2px(getContext(), DEFAULT_MARGIN_DIP);
        parentAvatarSize = ScreenUtil.dip2px(getContext(), DEFAULT_VIEW_SIZE_DIP);
        setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
    }

    public void setParentPadding(int padding){
        if (padding >= 0) {
            parentPadding = ScreenUtil.dip2px(getContext(), padding);
            setPadding(parentPadding, parentPadding, parentPadding, parentPadding);
        }
    }
    ...
    private void initOneRow(int curRow,boolean isFirst, int columnSize){
        try {
            ...
            for (int i = 0; i < columnSize; i++) {
                ...
                avatar.setImageResource(R.mipmap.tt_default_user_portrait_corner);
                avatar.setCorner(ScreenUtil.dip2px(getContext(), childCorner));
