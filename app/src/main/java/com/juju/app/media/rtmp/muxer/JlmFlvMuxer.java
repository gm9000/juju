package com.juju.app.media.rtmp.muxer;

import android.media.MediaCodec;
import android.media.MediaFormat;
import android.util.Log;

import com.juju.app.media.rtmp.JlmRtmpPublisher;
 ...
 *
 * Usage:
 *      muxer = new JlmRtmp("rtmp://ossrs.net/live/yasea");
 *      muxer.start();
 *
 ...
 *      muxer.release();
 */
public class JlmFlvMuxer {
    private volatile boolean connected = false;
    private String rtmpUrl;
    private JlmRtmpPublisher publisher;

    private Thread worker;
    private final Object txFrameLock = new Object();

    private JlmFlv flv = new JlmFlv();
    private boolean sequenceHeaderOk = false;
    private JlmFlvFrame videoSequenceHeader;
    private JlmFlvFrame audioSequenceHeader;
    private ConcurrentLinkedQueue<JlmFlvFrame> frameCache = new ConcurrentLinkedQueue<JlmFlvFrame>();

    private static final int VIDEO_TRACK = 100;
    private static final int AUDIO_TRACK = 101;
    private static final String TAG = "JlmFlvMuxer";

    /**
     * constructor.
     * @param handler the rtmp event handler.
     */
    public JlmFlvMuxer(RtmpPublisher.EventHandler handler) {
        publisher = new JlmRtmpPublisher(handler);
    }
    ...
    public int addTrack(MediaFormat format) {
        if (format.getString(MediaFormat.KEY_MIME).contentEquals(MediaFormat.MIMETYPE_VIDEO_AVC)) {
    ...
    private void sendFlvTag(JlmFlvFrame frame) throws IllegalStateException, IOException {
            ...
            public void run() {
                while (!Thread.interrupted()) {
                    ...
                    while (!frameCache.isEmpty()) {
                        JlmFlvFrame frame = frameCache.poll();
                        try {
                            ...
                            if (frame.type == JlmCodecFlvTag.Video && frame.avc_aac_type == JlmCodecVideoAVCType.SequenceHeader) {
                                videoSequenceHeader = frame;
                            } else if (frame.type == JlmCodecFlvTag.Audio && frame.avc_aac_type == 0) {
    ...
    public void stop() {
        ...
        Log.i(TAG, String.format("JlmFlvMuxer closed"));
    }
     ...
     */
    public class JlmUtils {

        public JlmAnnexbSearch srs_avc_startswith_annexb(ByteBuffer bb, MediaCodec.BufferInfo bi) {
            JlmAnnexbSearch as = new JlmAnnexbSearch();
            as.match = false;
    ...
    class JlmFlvFrame {
        ...
        public boolean is_keyframe() {
            return type == JlmCodecFlvTag.Video && frame_type == JlmCodecVideoAVCFrame.KeyFrame;
        }

        public boolean is_video() {
            return type == JlmCodecFlvTag.Video;
        }

        public boolean is_audio() {
            return type == JlmCodecFlvTag.Audio;
        }
    }
     ...
     */
    class JlmRawH264Stream {
        private JlmUtils utils;
        private final static String TAG = "JlmFlvMuxer";

        public JlmRawH264Stream() {
            utils = new JlmUtils();
        }

        public boolean is_sps(JlmFlvFrameBytes frame) {
            if (frame.size < 1) {
                return false;
            }
            return (frame.data.get(0) & 0x1f) == JlmAvcNaluType.SPS;
        }

        public boolean is_pps(JlmFlvFrameBytes frame) {
            if (frame.size < 1) {
                return false;
            }
            return (frame.data.get(0) & 0x1f) == JlmAvcNaluType.PPS;
        }

        public JlmFlvFrameBytes mux_ibp_frame(JlmFlvFrameBytes frame) {
            JlmFlvFrameBytes nalu_header = new JlmFlvFrameBytes();
            nalu_header.size = 4;
        ...
        public void mux_sequence_header(ByteBuffer sps, ByteBuffer pps, int dts, int pts, ArrayList<JlmFlvFrameBytes> frames) {
            ...
            if (true) {
                JlmFlvFrameBytes hdr = new JlmFlvFrameBytes();
                hdr.size = 5;
            ...
            // sps
            if (true) {
                JlmFlvFrameBytes sps_hdr = new JlmFlvFrameBytes();
                sps_hdr.size = 3;
                ...
                // sequenceParameterSetNALUnit
                JlmFlvFrameBytes sps_bb = new JlmFlvFrameBytes();
            ...
            // pps
            if (true) {
                JlmFlvFrameBytes pps_hdr = new JlmFlvFrameBytes();
                pps_hdr.size = 3;
                ...
                // pictureParameterSetNALUnit
                JlmFlvFrameBytes pps_bb = new JlmFlvFrameBytes();
        ...
        public JlmFlvFrameBytes mux_avc2flv(ArrayList<JlmFlvFrameBytes> frames, int frame_type, int avc_packet_type, int dts, int pts) {
            JlmFlvFrameBytes flv_tag = new JlmFlvFrameBytes();
            ...
            for (int i = 0; i < frames.size(); i++) {
                JlmFlvFrameBytes frame = frames.get(i);
            ...
            // set the rtmp header
            flv_tag.data.put((byte)((frame_type << 4) | JlmCodecVideo.AVC));
        ...
        public JlmFlvFrameBytes annexb_demux(ByteBuffer bb, MediaCodec.BufferInfo bi) throws IllegalArgumentException {
            JlmFlvFrameBytes tbb = new JlmFlvFrameBytes();

            while (bb.position() < bi.size) {
                // each frame must prefixed by annexb format.
                // about annexb, @see H.264-AVC-ISO_IEC_14496-10.pdf, page 211.
                JlmAnnexbSearch tbbsc = utils.srs_avc_startswith_annexb(bb, bi);
                if (!tbbsc.match || tbbsc.nb_start_code < 3) {
                    Log.e(TAG, "annexb not match.");
                   JlmFlvMuxer.srs_print_bytes(TAG, bb, 16);
                ...
                int pos = bb.position();
                while (bb.position() < bi.size) {
                    JlmAnnexbSearch bsc = utils.srs_avc_startswith_annexb(bb, bi);
                    if (bsc.match) {
                        break;
                    }
                    bb.get();
                ...
                if (bb.position() < bi.size) {
                    ...
                    JlmFlvMuxer.srs_print_bytes(TAG, tbbs, 16);
                    JlmFlvMuxer.srs_print_bytes(TAG, bb.slice(), 16);
                }
                break;
            }
...
//                String hex = Integer.toHexString(bb.get(0) & 0xFF);
//                if (hex.length() == 1) {
//                    hex = '0' + hex;
//                }
//                System.out.println("------------------------------------------:"+hex.toUpperCase());
//                byte ch = (byte)(0x00000013 & 0xf8);
                ...
                if (asample_rate == JlmCodecAudioSampleRate.R22050) {
                    samplingFrequencyIndex = 0x07;
                } else if (asample_rate == JlmCodecAudioSampleRate.R11025) {
            ...
            JlmFlvFrameBytes tag = new JlmFlvFrameBytes();
            tag.data = ByteBuffer.wrap(frame);
            tag.size = frame.length;

            rtmp_write_packet(JlmCodecFlvTag.Audio, dts, 0, aac_packet_type, tag);
        }
        ...
        public void writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) throws IllegalArgumentException {
            ...
            ArrayList<JlmFlvFrameBytes> ibps = new ArrayList<JlmFlvFrameBytes>();
            int frame_type = JlmCodecVideoAVCFrame.InterFrame;

            // send each frame.
            while (bb.position() < bi.size) {
                JlmFlvFrameBytes frame = avc.annexb_demux(bb, bi);

                // 5bits, 7.3.1 NAL unit syntax,
                // H.264-AVC-ISO_IEC_14496-10.pdf, page 44.
                //  7: SPS, 8: PPS, 5: I Frame, 1: P Frame
                int nal_unit_type = (int)(frame.data.get(0) & 0x1f);
                if (nal_unit_type == JlmAvcNaluType.SPS || nal_unit_type == JlmAvcNaluType.PPS) {
                    Log.i(TAG, String.format("annexb demux %dB, pts=%d, frame=%dB, nalu=%d", bi.size, pts, frame.size, nal_unit_type));
                }

                // for IDR frame, the frame is keyframe.
                if (nal_unit_type == JlmAvcNaluType.IDR) {
                    frame_type = JlmCodecVideoAVCFrame.KeyFrame;
                }

                // ignore the nalu type aud(9)
                if (nal_unit_type == JlmAvcNaluType.AccessUnitDelimiter) {
                    continue;
                }
                ...
                // ibp frame.
                JlmFlvFrameBytes nalu_header = avc.mux_ibp_frame(frame);
        ...
        private void write_h264_sps_pps(int dts, int pts) {
            ...
            ArrayList<JlmFlvFrameBytes> frames = new ArrayList<JlmFlvFrameBytes>();
            avc.mux_sequence_header(h264_sps, h264_pps, dts, pts, frames);

            // h264 packet to flv packet.
            int frame_type = JlmCodecVideoAVCFrame.KeyFrame;
            int avc_packet_type = JlmCodecVideoAVCType.SequenceHeader;
            JlmFlvFrameBytes flv_tag = avc.mux_avc2flv(frames, frame_type, avc_packet_type, dts, pts);

            // the timestamp in rtmp message header is dts.
            rtmp_write_packet(JlmCodecFlvTag.Video, dts, frame_type, avc_packet_type, flv_tag);
        ...
        private void write_h264_ipb_frame(ArrayList<JlmFlvFrameBytes> ibps, int frame_type, int dts, int pts) {
            ...
            int avc_packet_type = JlmCodecVideoAVCType.NALU;
            JlmFlvFrameBytes flv_tag = avc.mux_avc2flv(ibps, frame_type, avc_packet_type, dts, pts);

            if (frame_type == JlmCodecVideoAVCFrame.KeyFrame) {
        ...
        private void rtmp_write_packet(int type, int dts, int frame_type, int avc_aac_type, JlmFlvFrameBytes tag) {
            JlmFlvFrame frame = new JlmFlvFrame();
            frame.tag = tag;
            frame.type = type;
            frame.dts = dts;
